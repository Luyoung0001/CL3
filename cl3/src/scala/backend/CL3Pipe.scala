...
class PipeInput() extends Bundle {
  ...
  val irq      = Input(Bool())
  val e1_flush = Input(Bool())
  val e2_flush = Input(Bool())
...
class CL3Pipe(pipeID: Int) extends Module {
  ...
  when(io.in.issue.fire && !e1_flush && !e1_stall) {
    ...
    e1_q.except := MuxCase(
      ...
      Seq(
        ...
        branch_misaligned_w      -> ExceptionInfo.apply("h10".U(6.W), io.in.issue.info.pc, io.in.exu(0).br.pc)
      )
    )

    e1_q.npc := MuxCase(
      io.in.issue.info.pc + 4.U,
      Seq(
  ...
  val e2_flush = e1_stall && !e2_stall || e1_flush && !e2_stall || io.in.e2_flush
  ...
  when(!e2_flush && !e2_stall) {
    ...
    e2_q.result    := MuxCase(
      io.in.exu(0).result,
      Seq(e1_q.info.isDIV -> io.in.div.result, e1_q.info.isCSR -> io.in.csr.rdata)
    )
    e2_q.csr.wen   := io.in.csr.wen && e1_q.info.isCSR
    e2_q.csr.wdata := io.in.csr.wdata
    e2_q.except    := Mux(!e1_q.except.valid && e1_q.info.isCSR, io.in.csr.except, e1_q.except)
  }
  ...
  io.out.e2.except := Mux(e2_is_waiting_lsu, io.in.lsu.except, e2_q.except)
  ...
  val wb_flush = !wb_stall && e2_stall || io.in.wb_flush
  ...
  when(!wb_flush && !wb_stall) {
    wb_q               := e2_q
    wb_q.except        := io.out.e2.except
    wb_q.result        := io.out.e2.result
    wb_q.mem.cacheable := Mux(e2_q.isMem, io.in.lsu.cacheable, true.B) // Difftest signal
  ...
  io.out.wb.commit        := wb_q.valid && !wb_stall && !io.in.wb_flush

  io.out.wb.stall := wb_is_waiting_lsu && !io.in.lsu.valid

  io.out.wb.info.wen := wb_q.info.wen && !io.out.wb.except.valid
  io.out.wb.csr.wen  := wb_q.csr.wen && !io.out.wb.except.valid

  val wb_except = Mux(wb_is_waiting_lsu, io.in.lsu.except, wb_q.except)
  io.out.wb.except      := wb_except
  io.out.wb.except.code := wb_except.code & Fill(6, io.out.wb.commit) // override

  import CSRTrapNO._

  // TODO: use BoringUtil
  io.out.wb.npc := Mux(
    io.out.wb.except.valid,
    Mux(io.out.wb.except.code === EXCEPTION_ERET_M, io.in.epc, io.in.tvec), // TODO: add U-Mode and S-Mode eret
    wb_q.npc
  ) // Difftest only. Optimized away when disabled
