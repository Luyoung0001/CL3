...
//-----------------------------------------------------------------
// Tag RAM 0KB (256 x 20)
// Mode: Write First
//-----------------------------------------------------------------
  // val addr = Mux(io.p1.wen, io.p1.addr, io.p0.addr)
  val mem = SyncReadMem(p.sets, UInt((p.tagEntryW - 1).W))
...
class dcache_core_dirty_ramDev(p: DCacheParams) extends Module {
  val io = IO(new DCacheDirtyRamIO(p))

  // One bit per set stored in registers; avoids sharing the tag RAM port.
  val mem = SyncReadMem(p.sets, Bool())

  val dirty_r = mem.read(io.p0.addr, true.B)
  io.p0.rdata := dirty_r
