...
//-----------------------------------------------------------------
  ...
  when(state_q === STATE_LOOKUP) {
    ...
    when((mem_rd_m_q || mem_amo_m_q || (mem_wr_m_q =/= 0.U)) && !tag_hit_any_m_w) {
      ...
      (mem_wr_m_q.orR) && cpureq.rd && ((cpureq.addr(3, 2) === mem_addr_m_q(3, 2)) || (cpureq
        .addr(6, 5) === mem_addr_m_q(6, 5))) && mem_addr_m_q(31, 28) =/= 0.U
    ) {
      mem_accept_r := false.B
    }.otherwise {
      mem_accept_r := true.B
...
// reg [3:0] data0_write_m_r;
  ...
  for (i <- 0 until p.ways) {
    ...
    val data_in_m_w = Mux(state_q === STATE_REFILL, pmem_read_data_w, Mux(mem_amo_m_q, atoData, mem_data_m_q))
...
//-----------------------------------------------------------------
  ...
  reservedValid := Mux(mem_lr_m_q, true.B, Mux(mem_sc_m_q, false.B, reservedValid))
...
//-----------------------------------------------------------------
  // mem_data_m_q : rs2
  atoData := MuxLookup(mem_amoop_m_q, 0.U(p.dataW.W))(
    Seq(
...
//-----------------------------------------------------------------
  ...
  pmem_addr_w       := Mux(
    pmem_len_w.orR,
    Mux(pmem_rd_w, Cat(mem_addr_m_q(31, p.tagReqLineL), 0.U(p.offBits.W)), Cat(evict_addr_w, 0.U(p.offBits.W))),
    pmem_addr_q
  )
